...
%Read image and show the "noisy1.png"
image1 = imread("noisy1.png");


%Find its rows and columns
[m, n] = size(image1);

%Creating a new image. (Extra 2 colums and 2 row then image2) For Edge Detection   
paddingImageForEdge = uint8(zeros(m+2, n+2));
paddingImageForEdge(2:m+1, 2:n+2-1) = double(image1(:,:)); %It essentially copies the content of image1
...
%For determine the noise type, take a block in image1 and pilot it
block = image1(10:659,992:1227);


figure()
subplot(1,2,1)
imshow(block)
title("Block in noisy1.png")
subplot(1,2,2)
imhist(block);
...
paddingImage(2:m+1, 2:n+2-1) = double(image1(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
...
%Creating a new image. (Extra 2 colums and 2 row then image2)  
paddingImage = uint8(zeros(m+2, n+2));
paddingImage(2:m+1, 2:n+2-1) = double(Image1Output(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
...
title("Recovered1.png")
imwrite(uint8(Image1Output), 'Recovered1.png'); %Write Recovered image
...
title("Difference (subtraction) between edges of noisy1 and reconstructed images.")

%% 
clear all;clc;

%Read image and show the "noisy2.png"
image2 = imread("noisy2.png");
subplot(1,5,1)
imshow(image2); 
title('Step 1');
...
paddingImageForEdge(2:row+1, 2:col+2-1) = double(image2(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
...
%For determine the noise type, take a block in image1 and pilot it
block = image2(1389:1573,456:1102);
...
%          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
% When I look to histogram of block of image2, I understed that it is not
% addative noise, because the histogram grafic not like the addative
% noise's histogram graphic.  ->> periodic noise 
% So, I should Fourier Transform...


toFrequencyDomain = (fft2(image2)./(row*col));
...
imwrite(G3,"recovered2.png"); %Writing Recovered image


%Creating a new image. (Extra 2 colums and 2 row then image2)  
paddingImage = uint8(zeros(row+2, col+2));
paddingImage(2:row+1, 2:col+2-1) = double(G3(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
...
subplot(1,3,3)
title("Difference (subtraction) between edges of noisy2 and reconstructed images.")
imshow(double(difference))

%%
clear all;clc;

%Read image and show the "noisy3.png"
image3 = imread("noisy3.png");
subplot(1,5,1)
imshow(image3);
title('Step 1');
...
toFrequencyDomain = (fft2(image3)./(row*col));
centered = fftshift(toFrequencyDomain);
subplot(1,5,2)
imshow(log(centered),[])
title('Step 2')
...
%Creating a new image. (Extra 2 colums and 2 row then image2)  
paddingImageForEdge = uint8(zeros(row+2, col+2));
paddingImageForEdge(2:row+1, 2:col+2-1) = double(image3(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
...
paddingImageForRecover(2:row+1, 2:col+2-1) = double(G3(:,:)); %It essentially copies the content of image1
% into the center of paddingImage.
